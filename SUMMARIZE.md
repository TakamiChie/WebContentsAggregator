...
```shell
# 文字起こしがある未生成の記事を生成・保存（記事選定用ハッシュタグなし）
php summarize.php
...
全件処理は `transcript_vtt` がNULL・空文字列でない記事を対象とします。`--nodb` 未指定時は、`llm_summary` と `llm_tags` の両方に値がある記事をスキップします（`--content` 指定時も同様）。NULL・空文字列・空白のみは未生成として扱い、タグの `[]` は生成済みとして扱います。スキップした記事ではLLM呼び出し・DB更新・待機を行いません。`--nodb` 指定時は既存の生成結果に関係なく処理します。WebVTTの時刻・キューID・注記・装飾タグを取り除き、発話本文を順序どおり送信します。入力は切り詰めません。長文でモデルのコンテキスト長を超える場合は、LM Studio側のコンテキスト長またはモデルを調整してください。出力の上限は2048トークンです。上限に達して途中で切れた応答は保存しません。HTTP接続は10秒、生成は600秒でタイムアウトします。
